...
class TerminalService {
  ...
  private folders: Set<string> = new Set(); // <--- new
  private listeners: TerminalEventListener[] = [];

  constructor() {
    this.initializeFileSystem();
    console.log(
      "%c[Terminal Service] Initialized",
      "color: #0dbc79; font-weight: bold"
    );
    console.log(
      "%cGit commands available: init, status, add, commit, log, branch, clone",
      "color: #11a8cd"
    );
    this.folders.add("~/project");
  }
  ...
  private getCurrentFiles(): string[] {
    ...
    // get files directly under this dir
    ...
    // get folders directly under this dir
    const dirs = Array.from(this.folders)
      .filter((path) => path.startsWith(prefix) && path !== this.currentDir)
      .map((path) => path.replace(prefix, "").split("/")[0]) // only top-level
  ...
  private gitStatus(): string {
    ...
    if (
      repo.staged.size === 0 &&
      repo.modified.size === 0 &&
      repo.untracked.size === 0
    ) {
  ...
  private gitCommit(args: string[]): string {
    ...
    return `[${repo.branch} ${commit.hash}] ${message}\n ${
      commit.files.length
    } file${commit.files.length !== 1 ? "s" : ""} changed`;
  }
  ...
  private handleLs(): string {
    const items = this.getCurrentFiles();
    if (!items.length) return "";
    // mark directories with a trailing slash
    return items
      .map((item) =>
        this.folders.has(`${this.currentDir}/${item}`) ? item + "/" : item
      )
      .join("\n");
  }
  ...
  private handleMkdir(dirname?: string): string {
    ...
    const fullPath = `${this.currentDir}/${dirname}`;
    this.folders.add(fullPath); // track folder
    console.log(
      "%c[Terminal] Created directory: " + dirname,
      "color: #0dbc79; font-weight: bold"
    );
  ...
  private handleTouch(filename?: string): string {
    ...
    console.log(
      "%c[Terminal] Created file: " + filename,
      "color: #0dbc79; font-weight: bold"
    );
  ...
  private getHelp(): string {
    const gitCommands: [string, string][] = [
    ...
    const format = (cmds: [string, string][]) =>
      cmds.map(([cmd, desc]) => `${cmd.padEnd(20)} - ${desc}`).join("\n");
...
Available commands:
  ...
  addFileFromUI(filename: string): void {
    ...
    if (repo) {
      console.log(
        "%c[Terminal] UI file added to Git: " + filename,
  ...
  removeFileFromUI(filename: string): void {
    ...
    if (repo) {
      console.log(
        "%c[Terminal] UI file removed from Git: " + filename,
        "color: #f14c4c; font-weight: bold"
      );
      repo.untracked.delete(filename);
  ...
  modifyFileFromUI(filename: string): void {
    ...
    if (repo) {
      console.log(
        "%c[Terminal] UI file modified: " + filename,
        "color: #f5f543; font-weight: bold"
      );
